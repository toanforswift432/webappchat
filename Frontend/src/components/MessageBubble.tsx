...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const renderStatus = () => {
    ...
    switch (message.status) {
      case 'sent':
        return (
          <Check className="w-3 h-3 text-gray-300 dark:text-gray-400 ml-1" />);

      case 'delivered':
        return (
          <CheckCheck className="w-3 h-3 text-gray-300 dark:text-gray-400 ml-1" />);

      case 'seen':
        return (
          <CheckCheck className="w-3 h-3 text-blue-200 dark:text-blue-300 ml-1" />);
  ...
  const renderTextWithMentions = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@')) {
        ...
        const isUser = Object.values(users).some(
      ...
      <div
        ...
        <button
          ...
          className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] bg-white dark:bg-gray-700 border shadow-sm dark:shadow-none transition-colors
              ${userIds.includes('user-me') ? 'border-primary bg-primary-light dark:bg-primary-dark-light text-primary dark:text-blue-400' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600'}
            `}
          title={userIds.map((id) => users[id]?.name || id).join(', ')}>
    ...
    if (message.isRecalled) {
      return (
        <span className="italic opacity-70">
          {isOwn ? t('chat.youUnsent') : `${sender?.name} ${t('chat.unsent')}`}
        </span>);
      ...
      case 'file':
        return (
          <div
            ...
            <div
              className={`p-2 rounded-full ${isOwn ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-700'}`}>
              
              <FileText
                className={`w-6 h-6 ${isOwn ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`} />
            ...
            <button
              ...
              <Download
                className={`w-4 h-4 ${isOwn ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`} />
      ...
      case 'poll':
        ...
        return (
          <div className="mt-1 mb-1 w-full sm:w-64">
                    ...
                    <div className="relative flex justify-between items-center text-sm z-10">
                      <div className="flex items-center gap-2">
                        ...
                        `}>
                          ...
                          {hasVoted &&
                          <Check
                            className={`w-3 h-3 ${isOwn ? 'text-primary' : 'text-white'}`} />

                          }
                        </div>
                        <span className={hasVoted ? 'font-medium' : ''}>
                          {opt.text}
                        </span>
                      </div>
            ...
            <div
              className={`text-xs mt-2 opacity-70 ${isOwn ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
  ...
  return (
    <motion.div
      ...
      <div
        className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[75%] md:max-w-[65%]`}>
        ...
        <div className="relative flex items-center">
          ...
          {isOwn && showActions && !message.isRecalled &&
          <div className="absolute right-full mr-2 flex items-center gap-1 bg-white dark:bg-gray-700 shadow-sm dark:shadow-none border border-gray-200 dark:border-gray-600 rounded-lg p-1 transition-colors">
              <button
          ...
          {!isOwn && showActions && !message.isRecalled &&
          <div className="absolute left-full ml-2 flex items-center gap-1 bg-white dark:bg-gray-700 shadow-sm dark:shadow-none border border-gray-200 dark:border-gray-600 rounded-lg p-1 z-10 transition-colors">
              <button
              onClick={() => setShowEmojiPicker(!showEmojiPicker)}
              className="p-1.5 text-gray-500 dark:text-gray-300 hover:text-primary dark:hover:text-blue-400 hover:bg-primary-light dark:hover:bg-gray-600 rounded transition-colors">
          ...
          <AnimatePresence>
            ...
            <motion.div
              ...
              className={`absolute -top-10 ${isOwn ? 'right-0' : 'left-0'} bg-white dark:bg-gray-700 shadow-lg dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-600 rounded-full px-2 py-1 flex gap-1 z-20 transition-colors`}>
