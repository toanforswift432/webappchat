...
export const MessageSearchPanel: React.FC<MessageSearchPanelProps> = ({ isOpen, onClose, conversationId }) => {
  ...
  friends.forEach((f) => {
    users[f.id] = { name: f.name, avatar: f.avatar };
  });
  ...
  useEffect(() => {
    ...
    // Use timeout to debounce and avoid infinite loop
    const timeoutId = setTimeout(() => {
      const q = query.toLowerCase();
      const filtered = allMessages.filter((m) => !m.isRecalled && m.content?.toLowerCase().includes(q));
      setResults(filtered);
    }, 300);

    return () => clearTimeout(timeoutId);
  }, [query]); // Remove allMessages from dependencies to avoid infinite loop
