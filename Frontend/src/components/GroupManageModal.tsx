...
import { removeConversation, removeMemberFromConv, renameConversation, upsertConversation, setActiveConversation } from '../store/slices/conversationSlice';
...
export const GroupManageModal: React.FC<GroupManageModalProps> = ({
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { /* SignalR GroupAvatarUpdated updates store */ }
    finally {
  ...
  const handleAddMember = async (userId: string) => {
    ...
    try {
      ...
      setAddingIds((prev) => { const s = new Set(prev); s.delete(userId); return s; });
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        ...
        >
          ...
          <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 dark:border-gray-700">
            ...
            <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex flex-col items-center py-5 border-b border-gray-100 dark:border-gray-700 gap-2">
            <div className="relative">
              {group.groupAvatar
                ? <img src={group.groupAvatar} alt={group.groupName} className="w-20 h-20 rounded-full object-cover ring-4 ring-white dark:ring-gray-700 shadow-md" />
                : <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary to-primary-hover flex items-center justify-center text-white text-2xl font-bold shadow-md">{(group.groupName ?? 'G').slice(0, 2).toUpperCase()}</div>
              }
              {isAdmin && (
                <button
            ...
            {isEditingName ? (
              <div className="flex items-center gap-2 mt-1">
                <input
                  ...
                  maxLength={50}
                />
                <button onClick={handleRename} className="p-1 text-primary rounded-full hover:bg-primary/10 transition-colors"><Check className="w-4 h-4" /></button>
                <button onClick={() => setIsEditingName(false)} className="p-1 text-gray-400 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X className="w-4 h-4" /></button>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 mt-1">
                <span className="font-semibold text-gray-900 dark:text-white">{group.groupName}</span>
                {isAdmin && (
                  <button onClick={() => setIsEditingName(true)} className="p-1 text-gray-400 hover:text-primary rounded-full transition-colors">
            ...
            {tab === 'members' && (
              <div className="space-y-0.5 px-2">
                {currentMembers.map((member) => {
                  ...
                  return (
                    <div key={member.id} className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="relative flex-shrink-0">
                          <img src={member.avatar} alt={member.name} className="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-gray-700 shadow-sm" />
                          {member.isOnline && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white dark:border-gray-800 rounded-full" />}
                        </div>
                        <div className="min-w-0">
                          ...
                          {isMemberAdmin
                            ? <div className="flex items-center gap-1 text-xs text-amber-500 font-medium"><Crown className="w-3 h-3" /><span>Admin</span></div>
            ...
            {tab === 'add' && isAdmin && (
              <div className="space-y-0.5 px-2">
                ...
                {friendsToAdd.map((friend) => (
                  <div key={friend.id} className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="relative flex-shrink-0">
                        <img src={friend.avatar} alt={friend.name} className="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-gray-700 shadow-sm" />
                        {friend.isOnline && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white dark:border-gray-800 rounded-full" />}
                      </div>
            ...
            >
              ...
              >
                <div className="flex flex-col items-center text-center gap-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${confirm.type === 'kick' ? 'bg-orange-100 dark:bg-orange-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                    <AlertTriangle className={`w-6 h-6 ${confirm.type === 'kick' ? 'text-orange-500' : 'text-red-500'}`} />
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {confirm.type === 'kick' && `Remove ${confirm.userName} from the group? They won't be able to see messages anymore.`}
                      {confirm.type === 'leave' && "Are you sure you want to leave this group?"}
                      {confirm.type === 'delete' && "This will permanently delete the group for all members. This action cannot be undone."}
                    </p>
                  </div>
                  <div className="flex gap-3 w-full pt-1">
                    ...
                    >
                      {isLoading
                        ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Processing...</span>
                        : confirm.type === 'kick' ? 'Remove'
