import { Conversation, Message, User, Account, FriendRequest, Contact, AppNotification } from "./types";
...
let conversationsStore: Conversation[] = [
  {
    ...
    groupAvatar: "https://ui-avatars.com/api/?name=PA&background=1877f3&color=fff",
    members: [users["user-me"], users["user-1"], users["user-2"], users["user-3"]],
...
export const fetchUsers = async (): Promise<User[]> => {
  return new Promise((resolve) => {
    setTimeout(() => resolve(Object.values(users).filter((u) => u.id !== CURRENT_USER_ID)), 200);
  });
};

export const createGroup = async (name: string, memberIds: string[]): Promise<Conversation> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const groupMembers = [users[CURRENT_USER_ID], ...memberIds.map((id) => users[id])];
...
export const updateGroupMembers = async (groupId: string, memberIds: string[]): Promise<Conversation> => {
...
export const toggleReaction = async (messageId: string, emoji: string, userId: string): Promise<void> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const msg = messagesStore.find((m) => m.id === messageId);
      if (msg) {
...
export const forwardMessage = async (messageId: string, targetConversationId: string): Promise<Message> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const convIndex = conversationsStore.findIndex((c) => c.id === targetConversationId);
      if (convIndex !== -1) {
...
export const votePoll = async (messageId: string, optionId: string, userId: string): Promise<void> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const msg = messagesStore.find((m) => m.id === messageId);
...
export const searchMessages = async (query: string): Promise<Message[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const results = messagesStore.filter((m) => m.type === "text" && m.content.toLowerCase().includes(lowerQuery));
      resolve(results);
    }, 300);
  });
};
...
export const fetchMessages = async (conversationId: string): Promise<Message[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const msgs = messagesStore.filter((m) => m.conversationId === conversationId);
      resolve(msgs);
    }, 300);
  });
};
...
export const sendMessage = async (message: Omit<Message, "id" | "timestamp" | "status">): Promise<Message> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const convIndex = conversationsStore.findIndex((c) => c.id === message.conversationId);
      if (convIndex !== -1) {
...
export const simulateReply = async (conversationId: string, senderId: string): Promise<Message> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const reply: Message = {
        id: `m${Date.now()}`,
...
// Auth API
export const loginAccount = async (email: string, password: string): Promise<Account> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const account = accountsStore.find((a) => a.email === email && a.password === password);
      if (account) {
...
export const registerAccount = async (email: string, password: string, name: string): Promise<Account> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
...
export const fetchContacts = async (): Promise<(Contact & { user: User })[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const requests = friendRequestsStore.filter(
        (fr) => fr.fromUserId === currentAccountId || fr.toUserId === currentAccountId,
      );
      resolve(requests);
    }, 300);
  });
...
export const sendFriendRequest = async (toUserId: string): Promise<FriendRequest> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const existingContact = contactsStore[currentAccountId]?.find((c) => c.userId === toUserId);
...
export const acceptFriendRequest = async (requestId: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (!contactsStore[request.fromUserId]) contactsStore[request.fromUserId] = [];
      ...
      const existingConv = conversationsStore.find(
        (c) =>
          !c.isGroup &&
          ((c.user.id === request.fromUserId && currentAccountId === request.toUserId) ||
            (c.user.id === request.toUserId && currentAccountId === request.fromUserId)),
      );
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (contactsStore[currentAccountId]) {
        contactsStore[currentAccountId] = contactsStore[currentAccountId].filter((c) => c.userId !== userId);
      }
      if (contactsStore[userId]) {
        contactsStore[userId] = contactsStore[userId].filter((c) => c.userId !== currentAccountId);
      }
      resolve();
    }, 300);
  });
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const results = Object.values(users).filter(
        (u) => u.id !== currentAccountId && u.name.toLowerCase().includes(lowerQuery),
      );
      resolve(results);
    }, 300);
  });
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const matchedUsers = Object.values(users)
        .filter((u) => u.id !== currentAccountId && u.name.toLowerCase().includes(lowerQuery))
        .slice(0, 5);
      ...
      const matchedMsgs = messagesStore
        .filter((m) => m.type === "text" && m.content.toLowerCase().includes(lowerQuery))
        .map((m) => {
          const conv = conversationsStore.find((c) => c.id === m.conversationId);
          const convName = conv ? (conv.isGroup ? conv.groupName : conv.user.name) : "Unknown";
